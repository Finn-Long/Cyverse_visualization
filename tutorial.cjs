...
(function() {
  ...
  function init() {
    ...
    select_Id();
    // slider_change();
    var slider = document.getElementById("milletone_slider");
    slider.onchange = function() {
      slider_change(this.value);
    }
  ...
  function slider_change(value) {
    ...
    slider_time.innerHTML = value + " minutes";
    progress_scatter_chart("scroll-chart1", false);
  }
  ...
  function progress_scatter_chart_2(canvas_id, num, status, skipped, orderby, order, id_list, animation_option) {
    ...
    //populate max_time
    for (let i = 0; i < active_ids.length; i ++) {
      var total_time = get_total_active_time(active_ids[i]);
      if (total_time > max_time) {
        max_time = total_time;
      }
    }

    //populate slider bar with max time
    var max_time_slider = ((max_time - max_time % 50)/50 + 1)*50;
    document.getElementById("milletone_slider").max = max_time_slider;
    if (document.getElementById("milletone_slider").value == -1) {
      document.getElementById("milletone_slider").min = 0;
      document.getElementById("milletone_slider").value = max_time_slider;
      document.getElementById("slider_time").innerHTML = document.getElementById("milletone_slider").value + " minutes";
    }

    var slider_value = document.getElementById("milletone_slider").value;
    for (let i = 0; i < active_ids.length; i ++) {
      var log_id = active_ids[i]
      //get cutoffs for slider value
      var max_x_ind = ind_within_time(log_id, slider_value);
      // console.log(log_id + " " + get_formated_quiz_name(quiz_names[cur_x_ind]) + slider_value);
    ...
    for (let i = 0; i < id_list.length; i ++) {
      ...
      if ((status == "stucked" && isStucked) || (status == "completed" && isCompleted) || status == "in progress" && is_in_progress || status == "N/A") {
        var id = id_list[i];
        var visited_millestones_data = visited_millestone_ind(id, max_x_ind);
        var temp_visited = [];
        var isSkipped = false;
  ...
  function ind_within_time(log_id, time_limit) {
    ...
    var cur_progress_ind = current_progress_ind(log_id);
    // var ind = ind_in_log_arr_helper(quiz_urls[cur_progress_ind-1], logs);
    // var time = active_time_between(0, ind, active_laps, logs);
    // console.log(log_id + " " + ind + " " + logs.length + " " + time);
    if (cur_x_ind == -3) {
  ...
  function visited_millestone_ind(logId, max_ind){
    const log_arr = filter_logs(logId, 0, 0, 0, 0, 0);
    if (log_arr == null) {
      throw "Couldn't find corresponding logId";
    }
